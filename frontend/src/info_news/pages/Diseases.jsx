// src/info_news/pages/Diseases.jsx
...
export default function Diseases() {
	...
	useEffect(() => {
		const fetchArticles = async () => {
			try {
				setLoading(true);
				// Gọi endpoint lấy category "diseases"
				const response = await API.get("/api/articles/diseases");
				if (response.data && response.data.data) {
					setArticles(response.data.data);
				} else {
					setError("No articles for Diseases.");
				}
			} catch (err) {
				console.error("Error fetching diseases articles:", err);
				setError("Unable to load Diseases articles. Please try again.");
			} finally {
				setLoading(false);
			}
		};
	...
	// Render danh sách Diseases
	const renderArticlesList = () => (
		<>
			<h1 className="mb-4">Diseases</h1>
						...
						<div className="card-body d-flex">
							...
							<div>
								<h3 className="card-title">
									Information on various diseases, treatments, and prevention
								</h3>
								<p className="card-text">
									Explore articles covering symptoms, treatments, and preventive
			...
			{currentArticles.length > 0 ? (
				<div className="row row-cols-1 row-cols-md-2 row-cols-lg-3 g-4">
					{currentArticles.map((article, idx) => (
						<div className="col" key={article.id || `disease-${idx}`}>
