// src/info_news/pages/Symptoms.jsx
...
export default function Symptoms() {
	...
	useEffect(() => {
		const fetchArticles = async () => {
			try {
				setLoading(true);
				// Endpoint for category "symptoms"
				const response = await API.get("/api/articles/symptoms");
				if (response.data && Array.isArray(response.data.data)) {
					setArticles(response.data.data);
				} else {
					setError("No articles found for Symptoms.");
				}
			} catch (err) {
				console.error("Error fetching symptoms articles:", err);
	...
	// Markdown rendering using ContentRenderer (very similar to other pages)
	...
	// Render the “list” of Symptoms articles
						...
						<div className="card-body d-flex align-items-center">
							...
							<div>
								<h3 className="card-title">
									Symptom overviews: recognition, causes, and when to seek help
								</h3>
								<p className="card-text">
									Learn about common symptom presentations, how to recognize
									warning signs, and basic self-care tips.
								</p>
							</div>
						</div>
					</div>
			...
			{currentArticles.length > 0 ? (
				<div className="row row-cols-1 row-cols-md-2 row-cols-lg-3 g-4">
					{currentArticles.map((article, idx) => (
						<div className="col" key={article.id || `symptom-${idx}`}>
				...
				<p>No Symptoms articles available.</p>
			)}
	...
	// Render a single article’s detail (with Markdown parsing)
	const renderArticleDetail = () => {
		if (!selectedArticle) return null;

		// Split the raw content into chunks separated by two or more newlines
