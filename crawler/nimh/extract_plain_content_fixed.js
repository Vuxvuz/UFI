...
const CONCURRENCY_LIMIT = 3; // Giảm số lượng request đồng thời
    ...
    inputData.map(item => 
      limit(async () => {
        ...
        try {
          ...
          // Set a shorter timeout and use 'domcontentloaded' instead of 'networkidle'
          ...
          // Try again with a simpler approach for failed pages
          try {
            console.log(`🔄 Retrying with simpler approach: ${item.title}`);
            const page = await browser.newPage();
            
            // Use a very basic approach for retries
            await page.goto(item.url, { timeout: 30000 });
            await page.waitForTimeout(2000);
            
            // Just grab the body text as fallback
            const simpleContent = await page.evaluate(() => {
            ...
            return {
              ...
              timestamp: new Date().toISOString(),
              note: "Retrieved with fallback method"
            };
          } catch (retryError) {
            console.error(`❌ Retry also failed for ${item.url}: ${retryError.message}`);
            return {
              title: item.title,
              url: item.url,
              content: `Error extracting content: ${error.message}. Retry failed: ${retryError.message}`,
              contentLength: 0,
              error: error.message,
              timestamp: new Date().toISOString()
            };
